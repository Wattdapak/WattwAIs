...
class PredictBillScreen extends StatefulWidget {
  ...
  @override
  State<PredictBillScreen> createState() =>
      _PredictBillScreenState();
}
        ...
        content: Form(
          ...
          child:
            ...
            child: Column(
              ...
              children: [
                TextFormField(controller: _newApplianceNameController,
                  ...
                  children: [
                    ...
                    Expanded(
                      child:
                          TextFormField(
                        controller:
                            _newApplianceQuantityController,
...
Future<void> _addApplianceFromDefault(
  ApplianceTemplate template,
) async {
  ...
  final newAppliance =
    ...
    watts:
        template.defaultWatts,

    quantity:
        template.defaultQuantity,

    hoursPerDay:
        template.defaultHoursPerDay,

    daysPerWeek:
        template.defaultDaysPerWeek,
  );
  ...
  Future<void> _addManualAppliance()
      async {

    if (!_applianceFormKey
        .currentState!
        .validate()) {
      return;
    }
    ...
    final newAppliance =
      ...
      hoursPerDay: int.tryParse(
            _newApplianceHoursController
                .text,
          ) ??
          1,

      daysPerWeek: int.tryParse(
            _newApplianceDaysController
                .text,
          ) ??
          7,
  ...
  Future<void> _editAppliance(
    ...
    final wattsController =
        TextEditingController(
      text: model.watts.toString(),
    );
    ...
    final hoursController =
        TextEditingController(
      text:
          model.hoursPerDay.toString(),
    );

    final daysController =
        TextEditingController(
      text:
          model.daysPerWeek.toString(),
    );
  ...
  Future<void> _predictBill() async {
    ...
    try {
      ...
      await BillService.saveBudget(
        double.parse(
          _budgetController.text,
        ),
      );

      await BillService.saveBaseRate(
        double.parse(
